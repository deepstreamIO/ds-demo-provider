...
FXPriceGenerator.prototype.getPrices = function( currencyPair, callback ) {
	var baseCurrency = currencyPair.substr( 0, 3 ),
		...
		sendPriceUpdate = this._sendPriceUpdate.bind( this, baseCurrency, termCurrency );

	this._intervals[ currencyPair ] = setInterval( sendPriceUpdate, 500 );
	this._sendPriceUpdate( baseCurrency, termCurrency );
};

FXPriceGenerator.prototype._sendPriceUpdate = function( baseCurrency, termCurrency ) {
	...
	callback( rate + Math.random() / 1000, rate - Math.random() / 1000 );
};
